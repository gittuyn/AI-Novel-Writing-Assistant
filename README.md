...
- 服务器版本可以在 GitHub Actions 云端完成 TypeScript 与前端构建，再通过经过校验的部署制品更新生产环境；低内存服务器不再需要承担源码编译，并会在切换前备份数据库、失败时恢复旧构建。
...
在 GitHub Actions 手动运行 `Server Build Artifact`，下载生成的
`ai-novel-server-<commit>.tar.gz` 到与该提交一致的服务器工作区，然后执行：

```bash
sudo scripts/deploy-server-artifact.sh /path/to/ai-novel-server-<commit>.tar.gz
```

脚本会校验制品与提交、在线备份 SQLite、替换构建目录、重启
`ai-novel.service` 并检查健康接口。生产服务器不会运行 TypeScript 或 Vite。
