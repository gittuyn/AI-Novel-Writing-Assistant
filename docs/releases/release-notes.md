...
- 服务器版本可以在 GitHub Actions 云端完成 TypeScript 与前端构建，再下载部署制品更新生产环境，低内存服务器不再需要承担源码编译。
- 部署前会校验制品提交与文件清单并在线备份 SQLite；切换后自动检查服务健康状态，失败时恢复旧构建。
- 云端构建不会自动连接生产服务器，制品下载与正式部署仍由管理员手动确认。
